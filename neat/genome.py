...
class DefaultGenome(object):
    ...
    def __init__(self, key):
        ...
        # Fitness results.
        self.fitness = None
        self.mo_fitness = None
        self.mo_rank = None
        self.f1f2 = None
        self.mo_rank = [-1,-1]
        # add
        self.speces_key = None
    ...
    def distance(self, other, config):
        ...
        if self.connections or other.connections:
            ...
            """
            Calculate the Euclidean distance between two genomes based on their fitness.

            Parameters:
            - self: A genome.
            - other: Another genome to compare with.
            - max_fitness_values: List of maximum fitness values for each objective [max_f1, max_f2, max_f3].

            Returns:
            - Euclidean distance based on fitness differences.
            
            squared_diff_sum = 0.0
            max_fitness_values = [1,1000,2.4]
            fitness_distance = 0.0

            if self.mo_fitness is not None and other.mo_fitness is not None:

                for i in range(len(self.mo_fitness)):
                    # 各目的の差の二乗を計算し、最大適応度で正規化
                    diff = abs(self.mo_fitness[i] - other.mo_fitness[i])
                    normalized_diff = (diff / max_fitness_values[i]) ** 2
                    squared_diff_sum += normalized_diff

                # 二乗和の平方根を距離として返す
                fitness_distance = squared_diff_sum ** 0.5
            

        distance = node_distance + connection_distance + fitness_distance*3
        """
