...
gdmean_list = []
gdstdev_list = []
def write_data_to_file(filename, generation, sid, gid, f1, f2, f3):
    with open(filename, "a") as file:
        file.write(f"{generation},{sid},{gid},{f1},{f2},{f3}\n")
...
class DefaultSpeciesSet(DefaultClassConfig):
    ...
    def speciate(self, config, population, generation):
        ...
        # collect graph
        # ファイル名の設定
        filename = "species_data.csv"
        # g sid gid f1 f2 f3
            # 種ごとの個体をループしてデータを収集しファイルに書き込み
        for sid, members in new_members.items():
            for gid in members:
                # ここで、個体の適応度を取得
                individual = population[gid]
                if population[gid].mo_rank[0] == 0:
                    f1 = individual.mo_fitness[0]
                    f2 = individual.mo_fitness[1]
                    f3 = individual.mo_fitness[2]

                    # ファイルにデータを書き込み
                    write_data_to_file(filename, generation + 1, sid, gid, f1, f2, f3)

        gdmean = mean(itervalues(distances.distances))
        gdstdev = stdev(itervalues(distances.distances))
        gdmean_list.append(gdmean)
        gdstdev_list.append(gdstdev)

        self.reporters.info(
            'Mean genetic distance {0:.3f}, standard deviation {1:.3f}'.format(gdmean, gdstdev))
        
        return gdmean,gdstdev
